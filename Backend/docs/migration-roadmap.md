...
| P-4 장소 상세 전부 | GET /saved-places/{placeId}, GET /saved-places/{placeId}/media, DELETE /saved-places/{placeId} | 장소 정보 / 관련 sharedMedias [{sharedMediaId, thumbnailUrl, author, caption, originalUrl, sharedAt}] | 상세만 신규 |
...
**3 히스토리.** 목록은 커서(sharedAt, sharedMediaId) 페이징이고 클라이언트가 이미 같은 방식(50건)이다. 재시도는 FAILED에서만 도메인이 허용하고 DB 조건부 UPDATE가 경쟁을 막는다(bean-fable 그대로).
