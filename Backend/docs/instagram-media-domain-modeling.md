...
- **다이어그램의 점선**: 집합체 경계를 넘는 id 참조(memberId, mediaId), 또는 인터페이스 구현 관계다.

## 전체 구조

집합체가 둘이다. 게시물(InstagramMedia)은 shortcode로 유일하고 추출 상태와 결과의 주인이며 사용자를 모른다. 공유 사건(SharedInstagramMedia)은 특정 사용자가 게시물을 공유한 일 한 번이고 같은 게시물을 다시 공유해도 새 사건으로 쌓인다.
...
SharedInstagramMedia                    공유 사건 하나. 재공유해도 새 사건으로 쌓여 이력이 남는다
├── memberId(Long)            소유자. 집합체 경계를 넘는 id 참조(감싸지 않기로 합의)
...
| SharedInstagramMedia | 클래스 | 공유 사건. 소유자, 게시물 id 참조, 받은 원본(InstagramUrl), 발급된 후보를 들고 decidePlaces 행동을 제공한다. 추출이 끝나지 않은 공유는 후보가 없어 결정을 거부한다 |
...
처음에는 표시용 값을 도메인 밖(record)에만 뒀는데, 장기(java-janggi) 방식을 따라 도메인 관계를 먼저 완성하고 영속화할 값을 매퍼가 고르는 구조로 방향을 정하면서 원칙을 바꿨다. 캡션, 썸네일, 작성자(author)는 MediaMetadata record로 게시물이 들되 검증하지 않는다. 제목은 따로 들지 않고 클라이언트가 caption 첫 줄을 잘라 보여주기로 러키와 합의했다. 원본 URL은 InstagramUrl이 직접 보관한다. 생성자가 원본(사실)을 받아 shortcode(해석)를 파싱하고 둘 다 들기 때문에 해석은 항상 그 원본의 해석이며 동등성은 shortcode만 본다. 처음에는 이 쌍을 SharedLink라는 별도 값 객체로 묶었는데, 원본과 해석을 따로 받는 생성자가 모순 쌍을 허용하는 구멍이 있어 러키와의 논의에서 InstagramUrl로 흡수했다. 게시물과 공유를 나누면서 소유자(memberId)와 InstagramUrl은 공유 행위의 사실이라 공유 사건 객체로 옮겨 갔고 게시물에는 MediaShortcode만 남았다. 조회(히스토리, 상세)는 여전히 record에서 응답으로 바로 간다.
